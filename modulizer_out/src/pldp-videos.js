...
class PLDPVideos extends Element {
  static get template() {
    return html`
    <firebase-query id="videos" path="/myPLDP/videos" order-by-child="PLDPUserID" equal-to="[[user.userID]]" data="{{videosData}}">
    </firebase-query>
        ...
        <template is="dom-repeat" items="[[videosData]]" as="item">
                ...
                <div class="w3-row w3-padding-small">
                  <div class="w3-left w3-center" style="width:150px; height: 150px">
                    <template is="dom-if" if="[[isEqual(item.UploadExternal, 'external')]]">
                      <img id="img[[item.dateRegistered]]" src\$="[[getYoutubeThumbnail(item.url)]]" alt="LP" style="width:150px; max-height: 150px; cursor: pointer;" data="[[item.url]]" on-click="_handleClick">
                    </template>
          ...
          <div class="ldp-modal" id="video-modal">
            <div class="ldp-modal-content" style="max-width:600px">
              <span on-tap="_closeModal" class="w3-button w3-display-topright w3-red w3-medium">×</span>
