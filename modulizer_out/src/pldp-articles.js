...
class PLDPArticles extends Element {
  static get template() {
    return html`
    <firebase-query id="articles" path="/myPLDP/news" order-by-child="PLDPUserID" equal-to="[[user.userID]]" limit-to-last="15" data="{{articlesData}}">
    </firebase-query>
                ...
                <div class="card-container fadeInUp" style="max-width:600px">
                  ...
                  <div class="container card-header w3-row">
                    ...
                    <template is="dom-if" if="[[isEqual(item.journalUserID, user.userID)]]">
                      ...
                      <paper-icon-button icon="delete" on-tap="_deleteArticle" id="del[[item.dateRegistered]]" data="[[item.newsID]]" style="color: rgb(150, 150, 150)"></paper-icon-button>
                    </template>
                  </div>
