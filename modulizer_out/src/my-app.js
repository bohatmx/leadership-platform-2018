...
import './daily-thoughts.js';
import './company-thoughts.js';
import './i-lead.js';
import './ldp-companies.js';
import './leadership-thoughts.js';
import './leadership-articles.js';
import './master-class.js';
import './topleader-thoughts.js';
import './leadership-podcasts.js';
import './leadership-videos.js';
import './leadership-ebooks.js';
import './company-profile.js';
import './company-staff.js';
import './thoughts-management.js';
import './read-articles.js';
import './add-thoughts.js';
import './add-article.js';
import './add-podcast.js';
import './add-video.js';
import './add-voicemail.js';
import './not-found.js';
import './sign-in.js';
import './my-pldp.js';
...
import './my-followers.js';
import './filtered-thoughts.js';
import './filtered-articles.js';
import './filtered-podcasts.js';
import './filtered-videos.js';
import './sidebar-menus.js';
import './terms-conditions.js';
import './company-analytics.js';
import './app-icons.js';
import './w3-styles.js';
import './shared-styles.js';
import './my-notifications.js';
...
class MyApp extends Element {
    ...
    ready(){
				...
				var a = dom(this.root).querySelectorAll('.drawer-list');
				var that = this;
				
				var closer = dom(this.root).querySelector('#close-side-bar');
            ...
            if($(closer).is(':visible')){
				...
				// const app2 = firebase.app("[SHARED]");

				// console.log("Firebase: ", firebase.database(app2).app);
    }
    ...
    _listclicked(){
				...
				if(this.listclicked === true){
            setTimeout(myFunction, 500);
            function myFunction(){
                that._closeNotificationsModal();
            }
				}
    }

    _openWelcomePage(){
				let modal = dom(this.root).querySelector('#welcomePage');
				this.loadPage('welcome-page');
				$(modal).css('display', 'block');
				this._closeFirstPage();
    }
    _closeWelcomePage(){
				let modal = dom(this.root).querySelector('#welcomePage');
    ...
    _openFirstPage(){
				let modal = dom(this.root).querySelector('#firstPage');
				$(modal).css('display', 'block');
    }
    _closeFirstPage(){
				let modal = dom(this.root).querySelector('#firstPage');
            ...
            database.ref('/users/' + this._user.userID).child('welcomePage').set(true);
				}
    }
    ...
    _openUserProfile(){
            ...
            this.addIOSBugfix();
				...
				let modal = dom(this.root).querySelector('#userProfile');
				this.loadPage('user-profile');
				$(modal).css('display', 'block');
    }
    ...
    _toArray(obj) {
            ...
            return Object.keys(obj).map(function(key) {
                if(obj[key].seen == false) {
                    unread++;
                    if(unread > 20) that.unreadstring = "20+";
                    else that.unreadstring = ""+unread; 
    ...
    sendFeedback(){
				...
				var feedbackCategory = this.$.feedbackCategory.selected;

				var nav = window.navigator;
				var userPlatform = "";
				userPlatform += "Browser Name: " + this.userBrowser() + " ==> ";
				userPlatform += "Browser Version: " + nav.appVersion + " ==> ";
				userPlatform += "Browser Language: " + nav.language + " ==> ";
				userPlatform += "Browser Online: " + nav.onLine + " ==> ";
				userPlatform += "Platform: " + nav.platform;

				var messageVal = this.$.feedbackMessage.validate();
				var subjectVal = this.$.feedbackSubject.validate();

				if(feedbackCategory == ""){
            this.showSnackBar("Feedback Category required!");
            return;
				}else if(!subject || !subjectVal){
            ...
            var data = {
                ...
                companyName: this._user.companyName,
                journalUserID: this._user.userID,
                journalUserName: this._user.firstName+" "+this._user.lastName,
                journalUserEmail: this._user.email,
            ...
            this.$.feedbackCategory.selected = "";

            this.showSnackBar("Feedback posted successfully!");

            // close modal
            var modal = dom(this.root).querySelector('#send-feedback');
    ...
    _openModal(e){
            ...
            this.addIOSBugfix();
				...
				if(modal_name == "my-notifications"){
            console.log("opening notifications");
            this._captureClicks("openedNotificationsPage");
            this.listclicked = false;
				}
				var modal = dom(this.root).querySelector('#'+modal_name);
				this.loadPage(modal_name);
				modal.style.display = 'block';
    }

    _openFeedbackModal(e){
				// ios 11 bug caret position
				if ( iOS && iOS11 ) {
            this.addIOSBugfix();
				}
				...
				var modal = dom(this.root).querySelector('#'+modal_name);
				modal.style.display = 'block';
				this._captureClicks("openedFeedbackPage");
    }
    _closeNotificationsModal(e){
				var modal = dom(this.root).querySelector('#my-notifications');
				modal.style.display = 'none';
            ...
            console.log("Error updating user notifications:", error);
				...
				database.ref('/users/'+this._userID+'/pldpnotifications').orderByChild("seen").equalTo(seen).once('value').then((data)=>{
            var myNotifications = data.val();
            if(myNotifications === null){
                console.log("No notifications");
            }else{
    ...
    _pldpnotificationSeen(nots){
				...
				$.each(nots, function(n, nt){
            myNotifications['/users/'+_userID+'/pldpnotifications/'+n+'/seen'] = true
				});
    ...
    isEqual(a, b) {
				...
				if((b == "12345" && a !=="-KgsUcgfo7z1U9MXgd9i") && (b == "12345" && a !=="-LDVbbRyIMhukVtTVQ0n") && (b == "12345" && a !=="-LEiZPT-C2PyLu_YLKNU") ){
            return true;
				}else return a === b;
    }
    ...
    _closeSidebarFromBody(){
				var backBTN = dom(this.root).querySelector('#close-side-bar');
				var sidebar = dom(this.root).querySelector('#sidebar');
    ...
    _routePageChanged(page) {
            ...
            // public app
            if(this._companyid == "-LEiZPT-C2PyLu_YLKNU"){
                this.page = 'i-lead';
                this._selected = 22;
            }else{
            ...
            this._selected = 22;
				 }else if(page == 'filtered-thoughts'){
            this._selected = 23;
				}else if(page == 'filtered-articles'){
            this._selected = 24;
				}else if(page == 'filtered-podcasts'){
            this._selected = 25;
				}else if(page == 'filtered-videos'){
            this._selected = 26;
				}else if(page == 'company-analytics'){
            this._selected = 27;
				}
    ...
    _pageChanged(page) {
				...
				this.page = page || 'topleader-thoughts';
				...
				this._captureClicks(page);
    }
    _captureClicks(page){
				if(this._user != undefined){
            var database = firebase.database();
            var userID = this._user.userID;
            var userName = this._user.firstName + " " + this._user.lastName;
            var myPhoto;
            var myCompanyID = this._user.companyID;
            var myCompanyName = this._user.companyName;
            var dateRegistered = Date.now();

            if(this._user.photoURL != undefined || this._user.photoURL != ""){
                myPhoto = this._user.photoURL;
            }else{
                myPhoto = "";
            }
    ...
    _loadElement1(e){
				...
				dom(this.root).querySelector('#paperSpeed').style.visibility = 'hidden';
    ...
    _removeLoadElement(){
				this._removeElement();
				dom(this.root).querySelector('#paperSpeed').style.visibility = 'visible';
    }
    ...
    _userDataChange(newData, oldData) {
				if(this._objLen(this._user) > 0){
            this._userID = this._user.userID;

            if(this._user.welcomePage != undefined) {
                if(this._user.welcomePage == false){
                    this._openFirstPage();
                }

            }else{
                console.log("First time to GLP")
                this._openFirstPage();
            }
            ...
            if((follow_id === null) || (follow_id == undefined)){
                // console.log("its null");
            }else{
                if(this._user.userID !== follow_id){
                    console.log("user ID: ", this._user.userID);
                    console.log("follow_id: ", follow_id);
                    var userRef = database.ref('users/' + follow_id);

                    userRef.on('value', function(data) {
                        var childData = data.val();
                        let journalUserName = childData.firstName + " "+childData.lastName;
                        ...
                        var mydata = {
                            "firstName":that._user.firstName,
                            "lastName":that._user.lastName,
                            "companyName":that._user.companyName,
                            "dateRegistered": followDate,
                            "photoURL":myPhoto
                        }

                        var updates = {}
                        updates["users/"+that._user.userID+"/following/"+follow_id]=theirdata;
                        updates["users/"+follow_id+"/follower/"+that._user.userID]=mydata;

                        database.ref().update(updates);
                        
                    });

                    let countFollowers = database.ref('users').child(follow_id).child('analytics').child('followers');

                    localStorage.removeItem("follow_id");

                }else{
                    console.log("cannot follow my self");
                }
            }
            ...
            // console.log(this._user.notifications);
            this._objLenNotifications(this._user.notifications);
				}
    ...
    _setAccountDefaults(){
				var primaryColor, companyLogo, menuTextColor, topBarColor, topBarColorText;
				...
				this._userLogo = localStorage.getItem("userLogo");

				topBarColor = localStorage.getItem("topBarColor");
				topBarColorText = localStorage.getItem("topBarColorText");

				this.updateStyles({'--app-theme-color': primaryColor});

				if(topBarColor != null || topBarColor != undefined){
            this.updateStyles({'--app-topbar-color': topBarColor});
            this.updateStyles({'--app-topbar-text': topBarColorText});

            menuTextColor = "color: "+topBarColorText;
            this.companyBgColor = "background-color: "+topBarColor;
				}else{
    ...
    _onCompanyDataReceived(newData, oldData) {
            ...
            */
            ...
            if(newData[0].topBarColor != undefined) var topBarColor = newData[0].topBarColor;
            else var topBarColor = newData[0].primaryColor;
            ...
            var topBarColorText = this.getContrastYIQ(topBarColor);
            if(topBarColorText == undefined) topBarColorText = "#000";

            this.updateStyles({'--app-topbar-text': topBarColorText});
            
            if(topBarColor != null || topBarColor != undefined){
                this.menuTextColor = "color: "+topBarColorText;
                this.companyBgColor = "background-color: "+topBarColor;
            }else{
            ...
            localStorage.setItem("topBarColorText", topBarColorText);
            localStorage.setItem("topBarColor", this.topBarColor);
            ...
            pendingMsgRef.on('child_removed', function(data) {
				...
				sessionStorage.setItem("user3", "I-Leader");
				...
				sessionStorage.setItem("DESC_ILEAD_DAILY_THOUGHT", "I-LEAD Thought");
    ...
    loadTerms(){
				...
				this.loadPage('terms-conditions');
    }
